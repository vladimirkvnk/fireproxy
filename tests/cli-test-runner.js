...
/**
 * Command-line test runner for Fireproxy
 * 
 * To use: node cli-test-runner.js
 */

// Mock browser API for command line testing
global.browser = {
  storage: {
    local: {
      get: async () => ({}),
...
// Import test modules
const { isValidDomain, normalizeDomain, extractDomainFromUrl } = require('../background/domain-management.js');
const { generatePACScript } = require('../background/pac-generator.js');
...
if (require.main === module) {
  runAllTests().then(results => {
    process.exit(results.failingTests > 0 ? 1 : 0);
  });
}