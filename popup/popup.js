...
function scanCurrentPage() {
  browser.tabs.query({ active: true, currentWindow: true })
    .then(tabs => {
      ...
      if (!domain) {
        showStatusMessage("Unable to extract domain from current page", true);
        return;
      }
    ...
    .then(response => {
      if (response && response.success) {
        updateDomainsList(response.domains);
        showStatusMessage(`Added domain from current page`);
      } else if (response && !response.success && response.message.includes("already exists")) {
        showStatusMessage("Domain already in proxy list");
      } else if (response) {
        showStatusMessage(response.message || "Failed to add domain", true);
      }
    })
